...
def test_currency_key_missing():
    ...
    data = {"operationAmount": {"amount": "100.00"}}
    assert get_transaction_amount(data) == 0.0
...
def test_trans_usd_to_rub(mock_convert):
    ...
    transactions = {"operationAmount": {"amount": "80.00", "currency": {"name": "USD", "code": "USD"}}}
...
def test_trans_eur_to_rub(mock_convert):
    ...
    transactions = {"operationAmount": {"amount": "92.74", "currency": {"name": "EUR", "code": "EUR"}}}
    result = get_transaction_amount(transactions)
...
def test_trans_no_rub_usd_eur():
    ...
    data = {"operationAmount": {"amount": "31957.58", "currency": {"name": "тенге.", "code": "KZT"}}}
    assert get_transaction_amount(data) == 0.0
